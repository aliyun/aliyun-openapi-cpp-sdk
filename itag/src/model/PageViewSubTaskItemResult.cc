...
PageViewSubTaskItemResult::PageViewSubTaskItemResult() :
	ServiceResult()
{}

PageViewSubTaskItemResult::PageViewSubTaskItemResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void PageViewSubTaskItemResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	auto allListNode = resultNode["List"]["MarkItemList"];
	for (auto resultNodeListMarkItemList : allListNode)
	{
		Result::MarkItemList markItemListObject;
		if(!resultNodeListMarkItemList["MarkItemId"].isNull())
			markItemListObject.markItemId = resultNodeListMarkItemList["MarkItemId"].asString();
		if(!resultNodeListMarkItemList["DataId"].isNull())
			markItemListObject.dataId = resultNodeListMarkItemList["DataId"].asString();
		if(!resultNodeListMarkItemList["DatasetId"].isNull())
			markItemListObject.datasetId = resultNodeListMarkItemList["DatasetId"].asString();
		if(!resultNodeListMarkItemList["State"].isNull())
			markItemListObject.state = resultNodeListMarkItemList["State"].asString();
		if(!resultNodeListMarkItemList["Mine"].isNull())
			markItemListObject.mine = std::stol(resultNodeListMarkItemList["Mine"].asString());
		if(!resultNodeListMarkItemList["RejectFlag"].isNull())
			markItemListObject.rejectFlag = std::stol(resultNodeListMarkItemList["RejectFlag"].asString());
		if(!resultNodeListMarkItemList["FixedFlag"].isNull())
			markItemListObject.fixedFlag = std::stol(resultNodeListMarkItemList["FixedFlag"].asString());
		if(!resultNodeListMarkItemList["AbandonFlag"].isNull())
			markItemListObject.abandonFlag = std::stol(resultNodeListMarkItemList["AbandonFlag"].asString());
		if(!resultNodeListMarkItemList["Weight"].isNull())
			markItemListObject.weight = std::stol(resultNodeListMarkItemList["Weight"].asString());
		if(!resultNodeListMarkItemList["DataMeta"].isNull())
			markItemListObject.dataMeta = resultNodeListMarkItemList["DataMeta"].asString();
		auto allMarkResultListNode = resultNodeListMarkItemList["MarkResultList"]["MarkResultListItem"];
		for (auto resultNodeListMarkItemListMarkResultListMarkResultListItem : allMarkResultListNode)
		{
			Result::MarkItemList::MarkResultListItem markResultListObject;
			if(!resultNodeListMarkItemListMarkResultListMarkResultListItem["UserMarkResultId"].isNull())
				markResultListObject.userMarkResultId = resultNodeListMarkItemListMarkResultListMarkResultListItem["UserMarkResultId"].asString();
			if(!resultNodeListMarkItemListMarkResultListMarkResultListItem["MarkResultId"].isNull())
				markResultListObject.markResultId = resultNodeListMarkItemListMarkResultListMarkResultListItem["MarkResultId"].asString();
			if(!resultNodeListMarkItemListMarkResultListMarkResultListItem["MarkTitle"].isNull())
				markResultListObject.markTitle = resultNodeListMarkItemListMarkResultListMarkResultListItem["MarkTitle"].asString();
			if(!resultNodeListMarkItemListMarkResultListMarkResultListItem["MarkResult"].isNull())
				markResultListObject.markResult = resultNodeListMarkItemListMarkResultListMarkResultListItem["MarkResult"].asString();
			if(!resultNodeListMarkItemListMarkResultListMarkResultListItem["QuestionId"].isNull())
				markResultListObject.questionId = resultNodeListMarkItemListMarkResultListMarkResultListItem["QuestionId"].asString();
			if(!resultNodeListMarkItemListMarkResultListMarkResultListItem["ResultType"].isNull())
				markResultListObject.resultType = resultNodeListMarkItemListMarkResultListMarkResultListItem["ResultType"].asString();
			if(!resultNodeListMarkItemListMarkResultListMarkResultListItem["Version"].isNull())
				markResultListObject.version = resultNodeListMarkItemListMarkResultListMarkResultListItem["Version"].asString();
			markItemListObject.markResultList.push_back(markResultListObject);
		}
		result_.list.push_back(markItemListObject);
	}
...
std::string PageViewSubTaskItemResult::getMsg()const
{
	return msg_;
}

bool PageViewSubTaskItemResult::getSucc()const
{
	return succ_;
}

std::string PageViewSubTaskItemResult::getErrorCode()const
{
	return errorCode_;
}

std::string PageViewSubTaskItemResult::getErrInfo()const
{
	return errInfo_;
}

std::string PageViewSubTaskItemResult::getCode()const
{
	return code_;
}

PageViewSubTaskItemResult::Result PageViewSubTaskItemResult::getResult()const
{
	return result_;
}
