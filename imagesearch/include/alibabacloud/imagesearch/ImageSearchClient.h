...
class ALIBABACLOUD_IMAGESEARCH_EXPORT ImageSearchClient
    : public RoaServiceClient {
public:
  ...
  typedef std::function<void(const ImageSearchClient *,
                             const Model::DeleteItemRequest &,
                             const DeleteItemOutcome &,
                             const std::shared_ptr<const AsyncCallerContext> &)>
      DeleteItemAsyncHandler;
  typedef Outcome<Error, Model::AddItemResult> AddItemOutcome;
  typedef std::future<AddItemOutcome> AddItemOutcomeCallable;
  typedef std::function<void(const ImageSearchClient *,
                             const Model::AddItemRequest &,
  ...
  typedef std::function<void(const ImageSearchClient *,
                             const Model::SearchItemRequest &,
                             const SearchItemOutcome &,
                             const std::shared_ptr<const AsyncCallerContext> &)>
      SearchItemAsyncHandler;

  ImageSearchClient(const Credentials &credentials,
                    const ClientConfiguration &configuration);
  ImageSearchClient(
      const std::shared_ptr<CredentialsProvider> &credentialsProvider,
      const ClientConfiguration &configuration);
  ImageSearchClient(const std::string &accessKeyId,
                    const std::string &accessKeySecret,
                    const ClientConfiguration &configuration);
  ~ImageSearchClient();
  DeleteItemOutcome deleteItem(const Model::DeleteItemRequest &request) const;
  void deleteItemAsync(
      const Model::DeleteItemRequest &request,
      const DeleteItemAsyncHandler &handler,
      const std::shared_ptr<const AsyncCallerContext> &context = nullptr) const;
  DeleteItemOutcomeCallable
  deleteItemCallable(const Model::DeleteItemRequest &request) const;
  AddItemOutcome addItem(const Model::AddItemRequest &request) const;
  void addItemAsync(
      const Model::AddItemRequest &request, const AddItemAsyncHandler &handler,
      const std::shared_ptr<const AsyncCallerContext> &context = nullptr) const;
  AddItemOutcomeCallable
  addItemCallable(const Model::AddItemRequest &request) const;
  SearchItemOutcome searchItem(const Model::SearchItemRequest &request) const;
  void searchItemAsync(
      const Model::SearchItemRequest &request,
      const SearchItemAsyncHandler &handler,
      const std::shared_ptr<const AsyncCallerContext> &context = nullptr) const;
  SearchItemOutcomeCallable
  searchItemCallable(const Model::SearchItemRequest &request) const;
