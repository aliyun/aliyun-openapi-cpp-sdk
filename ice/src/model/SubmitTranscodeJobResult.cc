...
SubmitTranscodeJobResult::SubmitTranscodeJobResult() :
	ServiceResult()
{}

SubmitTranscodeJobResult::SubmitTranscodeJobResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void SubmitTranscodeJobResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	for (auto transcodeParentJobNodeTranscodeJobListTranscodeJobListItem : allTranscodeJobListNode)
	{
		...
		if(!processConfigNode["IsInheritTags"].isNull())
			transcodeJobListItemObject.processConfig.isInheritTags = processConfigNode["IsInheritTags"].asString();
...
SubmitTranscodeJobResult::TranscodeParentJob SubmitTranscodeJobResult::getTranscodeParentJob()const
{
