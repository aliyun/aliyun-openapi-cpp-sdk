...
GetCloudAssetDetailResult::GetCloudAssetDetailResult() :
	ServiceResult()
{}

GetCloudAssetDetailResult::GetCloudAssetDetailResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void GetCloudAssetDetailResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	for (auto valueInstancesInstancesItem : allInstancesNode)
	{
		...
		if(!valueInstancesInstancesItem["AssetSubType"].isNull())
			instancesObject.assetSubType = std::stoi(valueInstancesInstancesItem["AssetSubType"].asString());
		...
		if(!valueInstancesInstancesItem["DetailLink"].isNull())
			instancesObject.detailLink = valueInstancesInstancesItem["DetailLink"].asString();
...
std::vector<GetCloudAssetDetailResult::InstancesItem> GetCloudAssetDetailResult::getInstances()const
{
	return instances_;
}

int GetCloudAssetDetailResult::getCount()const
{
	return count_;
}
