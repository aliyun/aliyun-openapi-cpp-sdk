...
void DescribeSuspEventQuaraFilesResult::parse(const std::string &payload)
{
	...
	for (auto valueQuaraFilesQuaraFile : allQuaraFilesNode)
	{
		...
		if(!valueQuaraFilesQuaraFile["Link"].isNull())
			quaraFilesObject.link = valueQuaraFilesQuaraFile["Link"].asString();
		...
		if(!valueQuaraFilesQuaraFile["InstanceId"].isNull())
			quaraFilesObject.instanceId = valueQuaraFilesQuaraFile["InstanceId"].asString();
