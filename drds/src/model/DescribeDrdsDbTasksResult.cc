...
void DescribeDrdsDbTasksResult::parse(const std::string &payload)
{
	...
	for (auto valueTasksTask : allTasksNode)
	{
		...
		if(!valueTasksTask["TaskDetail"].isNull())
			tasksObject.taskDetail = valueTasksTask["TaskDetail"].asString();
		...
		if(!valueTasksTask["ParentJobId"].isNull())
			tasksObject.parentJobId = valueTasksTask["ParentJobId"].asString();
		if(!valueTasksTask["ExpandType"].isNull())
			tasksObject.expandType = valueTasksTask["ExpandType"].asString();
		if(!valueTasksTask["TargetId"].isNull())
			tasksObject.targetId = std::stol(valueTasksTask["TargetId"].asString());
		if(!valueTasksTask["Label"].isNull())
			tasksObject.label = valueTasksTask["Label"].asString();
		if(!valueTasksTask["DetailTaskId"].isNull())
			tasksObject.detailTaskId = valueTasksTask["DetailTaskId"].asString();
		if(!valueTasksTask["TbComputeLength"].isNull())
			tasksObject.tbComputeLength = std::stoi(valueTasksTask["TbComputeLength"].asString());
		if(!valueTasksTask["DbComputeLength"].isNull())
			tasksObject.dbComputeLength = std::stoi(valueTasksTask["DbComputeLength"].asString());
