...
DescribeDownloadSQLLogsResult::DescribeDownloadSQLLogsResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void DescribeDownloadSQLLogsResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
...
std::vector<DescribeDownloadSQLLogsResult::Item> DescribeDownloadSQLLogsResult::getRecords()const
{
	return records_;
}
