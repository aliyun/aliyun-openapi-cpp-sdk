...
void GetRepositoryCommitResult::parse(const std::string &payload)
{
	...
	if(!resultNode["commentsCount"].isNull())
		result_.commentsCount = std::stol(resultNode["commentsCount"].asString());
	...
	if(!authorNode["externUid"].isNull())
		result_.author.externUid = authorNode["externUid"].asString();
	...
	if(!authorNode["tbUserId"].isNull())
		result_.author.tbUserId = authorNode["tbUserId"].asString();
	...
	if(!committerNode["externUid"].isNull())
		result_.committer.externUid = committerNode["externUid"].asString();
	...
	if(!committerNode["tbUserId"].isNull())
		result_.committer.tbUserId = committerNode["tbUserId"].asString();
...
std::string GetRepositoryCommitResult::getRequestId()const
{
	return requestId_;
}
