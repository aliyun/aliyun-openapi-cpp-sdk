...
GetMergeRequestResult::GetMergeRequestResult() :
	ServiceResult()
{}

GetMergeRequestResult::GetMergeRequestResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void GetMergeRequestResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	if(!resultNode["targetProjectNameWithNamespace"].isNull())
		result_.targetProjectNameWithNamespace = resultNode["targetProjectNameWithNamespace"].asString();
	if(!resultNode["targetProjectPathWithNamespace"].isNull())
		result_.targetProjectPathWithNamespace = resultNode["targetProjectPathWithNamespace"].asString();
	...
	for (auto resultNodereviewersreviewersItem : allreviewersNode)
	{
		...
		if(!resultNodereviewersreviewersItem["reviewTime"].isNull())
			reviewersItemObject.reviewTime = resultNodereviewersreviewersItem["reviewTime"].asString();
		if(!resultNodereviewersreviewersItem["hasCommented"].isNull())
			reviewersItemObject.hasCommented = resultNodereviewersreviewersItem["hasCommented"].asString() == "true";
	...
	auto todoListNode = resultNode["todoList"];
	auto allrequirementCheckItemsNode = todoListNode["requirementCheckItems"]["requirement_rule_items"];
	for (auto todoListNoderequirementCheckItemsrequirement_rule_items : allrequirementCheckItemsNode)
	{
		Result::TodoList::Requirement_rule_items requirement_rule_itemsObject;
		if(!todoListNoderequirementCheckItemsrequirement_rule_items["itemType"].isNull())
			requirement_rule_itemsObject.itemType = todoListNoderequirementCheckItemsrequirement_rule_items["itemType"].asString();
		if(!todoListNoderequirementCheckItemsrequirement_rule_items["pass"].isNull())
			requirement_rule_itemsObject.pass = todoListNoderequirementCheckItemsrequirement_rule_items["pass"].asString() == "true";
		result_.todoList.requirementCheckItems.push_back(requirement_rule_itemsObject);
	}
...
std::string GetMergeRequestResult::getRequestId()const
{
	return requestId_;
}

std::string GetMergeRequestResult::getErrorCode()const
{
	return errorCode_;
}

std::string GetMergeRequestResult::getErrorMessage()const
{
	return errorMessage_;
}

bool GetMergeRequestResult::getSuccess()const
{
	return success_;
}

GetMergeRequestResult::Result GetMergeRequestResult::getResult()const
{
	return result_;
}
