...
void CreatePipelineResult::parse(const std::string &payload)
{
	...
	if(!value["pipelinId"].isNull())
		pipelinId_ = std::stol(value["pipelinId"].asString());
	if(!value["requestId"].isNull())
		requestId_ = value["requestId"].asString();

}

std::string CreatePipelineResult::getRequestId()const
{
	return requestId_;
}

long CreatePipelineResult::getPipelinId()const
{
	return pipelinId_;
}
