...
#include <alibabacloud/rds/model/CreateSecretResult.h>
#include <json/json.h>
...
void CreateSecretResult::parse(const std::string &payload)
{
	...
	if(!value["SecretArn"].isNull())
		secretArn_ = value["SecretArn"].asString();
...
std::string CreateSecretResult::getSecretArn()const
{
	return secretArn_;
}
