...
RemoveAppResult::RemoveAppResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void RemoveAppResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
...
std::string RemoveAppResult::getRequestId()const
{
	return requestId_;
}

std::vector<std::string> RemoveAppResult::getResult()const
{
	return result_;
}
