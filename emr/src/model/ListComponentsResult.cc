...
#include <alibabacloud/emr/model/ListComponentsResult.h>
#include <json/json.h>
...
void ListComponentsResult::parse(const std::string &payload)
{
	...
	for (auto valueComponentsComponent : allComponentsNode)
	{
		...
		if(!valueComponentsComponent["Namespace"].isNull())
			componentsObject._namespace = valueComponentsComponent["Namespace"].asString();
...
std::vector<ListComponentsResult::Component> ListComponentsResult::getComponents()const
{
	return components_;
}

int ListComponentsResult::getTotalCount()const
{
	return totalCount_;
}

std::string ListComponentsResult::getNextToken()const
{
	return nextToken_;
}

int ListComponentsResult::getMaxResults()const
{
	return maxResults_;
}
