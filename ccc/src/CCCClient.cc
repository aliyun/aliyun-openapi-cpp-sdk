...
CCCClient::SaveStatsOutcome CCCClient::saveStats(const SaveStatsRequest &request) const
{
	auto endpointOutcome = endpointProvider_->getEndpoint();
	if (!endpointOutcome.isSuccess())
		return SaveStatsOutcome(endpointOutcome.error());

	auto outcome = makeRequest(endpointOutcome.result(), request);

	if (outcome.isSuccess())
		return SaveStatsOutcome(SaveStatsResult(outcome.result()));
	else
		return SaveStatsOutcome(outcome.error());
}

void CCCClient::saveStatsAsync(const SaveStatsRequest& request, const SaveStatsAsyncHandler& handler, const std::shared_ptr<const AsyncCallerContext>& context) const
{
	auto fn = [this, request, handler, context]()
	{
		handler(this, request, saveStats(request), context);
	};

	asyncExecute(new Runnable(fn));
}

CCCClient::SaveStatsOutcomeCallable CCCClient::saveStatsCallable(const SaveStatsRequest &request) const
{
	auto task = std::make_shared<std::packaged_task<SaveStatsOutcome()>>(
			[this, request]()
			{
			return this->saveStats(request);
			});
