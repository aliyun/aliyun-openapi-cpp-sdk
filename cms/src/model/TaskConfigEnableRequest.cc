...
using AlibabaCloud::Cms::Model::TaskConfigEnableRequest;
...
std::vector<long> TaskConfigEnableRequest::getIdList()const
{
	return idList_;
}
...
void TaskConfigEnableRequest::setIdList(const std::vector<long>& idList)
{
	...
	for(int i = 0; i!= idList.size(); i++)
		setParameter("IdList."+ std::to_string(i), std::to_string(idList.at(i)));
}
