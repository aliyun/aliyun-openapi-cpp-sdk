...
void ListInstancesResult::parse(const std::string &payload)
{
	...
	for (auto valueInstancesInstance : allInstancesNode)
	{
		...
		auto defaultEndpointNode = value["DefaultEndpoint"];
		if(!defaultEndpointNode["Endpoint"].isNull())
			instancesObject.defaultEndpoint.endpoint = defaultEndpointNode["Endpoint"].asString();
		if(!defaultEndpointNode["Status"].isNull())
			instancesObject.defaultEndpoint.status = defaultEndpointNode["Status"].asString();
