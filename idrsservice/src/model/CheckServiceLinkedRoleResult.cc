...
void CheckServiceLinkedRoleResult::parse(const std::string &payload)
{
	...
	if(!dataNode["HasRole"].isNull())
		data_.hasRole = dataNode["HasRole"].asString() == "true";
...
std::string CheckServiceLinkedRoleResult::getMessage()const
{
	return message_;
}

CheckServiceLinkedRoleResult::Data CheckServiceLinkedRoleResult::getData()const
{
	return data_;
}

std::string CheckServiceLinkedRoleResult::getCode()const
{
	return code_;
}
