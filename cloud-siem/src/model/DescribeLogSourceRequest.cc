...
using AlibabaCloud::Cloud_siem::Model::DescribeLogSourceRequest;

DescribeLogSourceRequest::DescribeLogSourceRequest()
    : RpcServiceRequest("cloud-siem", "2022-06-16", "DescribeLogSource") {
...
void DescribeLogSourceRequest::setLogType(const std::string &logType) {
  logType_ = logType;
  setBodyParameter(std::string("LogType"), logType);
}

std::string DescribeLogSourceRequest::getRegionId() const {
  return regionId_;
}

void DescribeLogSourceRequest::setRegionId(const std::string &regionId) {
  regionId_ = regionId;
