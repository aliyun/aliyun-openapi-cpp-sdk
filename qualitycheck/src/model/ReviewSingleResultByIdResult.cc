...
#include <alibabacloud/qualitycheck/model/ReviewSingleResultByIdResult.h>
#include <json/json.h>

using namespace AlibabaCloud::Qualitycheck;
using namespace AlibabaCloud::Qualitycheck::Model;

ReviewSingleResultByIdResult::ReviewSingleResultByIdResult() :
	ServiceResult()
{}

ReviewSingleResultByIdResult::ReviewSingleResultByIdResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void ReviewSingleResultByIdResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	if(!dataNode["Vid"].isNull())
		data_.vid = std::stoi(dataNode["Vid"].asString());
	...
	for (auto dataNodeDialoguesDialogue : allDialoguesNode)
	{
		...
		if(!dataNodeDialoguesDialogue["BeginTime"].isNull())
			dialogueObject.beginTime = std::stol(dataNodeDialoguesDialogue["BeginTime"].asString());
	...
	for (auto dataNodeHitRuleReviewInfoListHitRuleReviewInfo : allHitRuleReviewInfoListNode)
	{
		...
		if(!dataNodeHitRuleReviewInfoListHitRuleReviewInfo["Comments"].isNull())
			hitRuleReviewInfoObject.comments = dataNodeHitRuleReviewInfoListHitRuleReviewInfo["Comments"].asString();
...
std::string ReviewSingleResultByIdResult::getMessage()const
{
	return message_;
}

ReviewSingleResultByIdResult::Data ReviewSingleResultByIdResult::getData()const
{
	return data_;
}

std::string ReviewSingleResultByIdResult::getCode()const
{
	return code_;
}

bool ReviewSingleResultByIdResult::getSuccess()const
{
	return success_;
}
