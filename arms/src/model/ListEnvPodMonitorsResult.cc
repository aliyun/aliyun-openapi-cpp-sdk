...
ListEnvPodMonitorsResult::ListEnvPodMonitorsResult() :
	ServiceResult()
{}

ListEnvPodMonitorsResult::ListEnvPodMonitorsResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void ListEnvPodMonitorsResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	for (auto valueDataMonitoring : allDataNode)
	{
		...
		if(!valueDataMonitoring["PodMonitorName"].isNull())
			dataObject.podMonitorName = valueDataMonitoring["PodMonitorName"].asString();
...
std::string ListEnvPodMonitorsResult::getMessage()const
{
	return message_;
}

std::vector<ListEnvPodMonitorsResult::Monitoring> ListEnvPodMonitorsResult::getData()const
{
	return data_;
}

int ListEnvPodMonitorsResult::getCode()const
{
	return code_;
}
