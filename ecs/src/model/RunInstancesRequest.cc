...
void RunInstancesRequest::setNetworkInterface(const std::vector<NetworkInterface>& networkInterface)
{
	...
	for(int dep1 = 0; dep1!= networkInterface.size(); dep1++) {
		...
		for(int dep2 = 0; dep2!= networkInterfaceObj.ipv6Address.size(); dep2++) {
			setParameter(networkInterfaceObjStr + ".Ipv6Address."+ std::to_string(dep2), networkInterfaceObj.ipv6Address.at(dep2));
		}
	}
}
