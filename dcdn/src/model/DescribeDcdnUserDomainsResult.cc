...
void DescribeDcdnUserDomainsResult::parse(const std::string &payload)
{
	...
	for (auto valueDomainsPageData : allDomainsNode)
	{
		...
		if(!valueDomainsPageData["DomainId"].isNull())
			domainsObject.domainId = std::stol(valueDomainsPageData["DomainId"].asString());
		if(!valueDomainsPageData["Scene"].isNull())
			domainsObject.scene = valueDomainsPageData["Scene"].asString();
		if(!valueDomainsPageData["FunctionType"].isNull())
			domainsObject.functionType = valueDomainsPageData["FunctionType"].asString();
