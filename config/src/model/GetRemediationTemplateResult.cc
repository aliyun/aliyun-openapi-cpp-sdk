...
GetRemediationTemplateResult::GetRemediationTemplateResult() :
	ServiceResult()
{}

GetRemediationTemplateResult::GetRemediationTemplateResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void GetRemediationTemplateResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	for (auto valueRemediationTemplatesRemediationTemplate : allRemediationTemplatesNode)
	{
		...
		if(!valueRemediationTemplatesRemediationTemplate["ManagedRuleIdentifier"].isNull())
			remediationTemplatesObject.managedRuleIdentifier = valueRemediationTemplatesRemediationTemplate["ManagedRuleIdentifier"].asString();
...
std::vector<GetRemediationTemplateResult::RemediationTemplate> GetRemediationTemplateResult::getRemediationTemplates()const
{
