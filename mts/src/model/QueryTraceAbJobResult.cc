...
QueryTraceAbJobResult::QueryTraceAbJobResult() :
	ServiceResult()
{}

QueryTraceAbJobResult::QueryTraceAbJobResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void QueryTraceAbJobResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
...
std::string QueryTraceAbJobResult::getMessage()const
{
	return message_;
}

std::vector<QueryTraceAbJobResult::DataItem> QueryTraceAbJobResult::getData()const
{
	return data_;
}

long QueryTraceAbJobResult::getStatusCode()const
{
	return statusCode_;
}
