...
QueryMediaListByURLResult::QueryMediaListByURLResult() :
	ServiceResult()
{}

QueryMediaListByURLResult::QueryMediaListByURLResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void QueryMediaListByURLResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	auto allNonExistFileURLs = value["NonExistFileURLs"]["FileURL"];
	for (const auto &item : allNonExistFileURLs)
		nonExistFileURLs_.push_back(item.asString());

}

std::vector<QueryMediaListByURLResult::Media> QueryMediaListByURLResult::getMediaList()const
{
	return mediaList_;
}

std::vector<std::string> QueryMediaListByURLResult::getNonExistFileURLs()const
{
	return nonExistFileURLs_;
}
