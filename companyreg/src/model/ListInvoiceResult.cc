...
ListInvoiceResult::ListInvoiceResult() :
	ServiceResult()
{}

ListInvoiceResult::ListInvoiceResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void ListInvoiceResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	for (auto valueDataDataItem : allDataNode)
	{
		...
		if(!valueDataDataItem["InvoiceNo"].isNull())
			dataObject.invoiceNo = valueDataDataItem["InvoiceNo"].asString();
		...
		if(!valueDataDataItem["Use"].isNull())
			dataObject.use = valueDataDataItem["Use"].asString() == "true";
...
std::string ListInvoiceResult::getAmount()const
{
	return amount_;
}

std::vector<ListInvoiceResult::DataItem> ListInvoiceResult::getData()const
{
	return data_;
}

int ListInvoiceResult::getCount()const
{
	return count_;
}
