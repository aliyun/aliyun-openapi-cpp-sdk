...
void DescribeZonesResult::parse(const std::string &payload)
{
	...
	for (auto valueZonesZonesItem : allZonesNode)
	{
		...
		if(!valueZonesZonesItem["ModeCode"].isNull())
			zonesObject.modeCode = valueZonesZonesItem["ModeCode"].asString();
		if(!valueZonesZonesItem["Priority"].isNull())
			zonesObject.priority = valueZonesZonesItem["Priority"].asString();
		if(!valueZonesZonesItem["Status"].isNull())
			zonesObject.status = valueZonesZonesItem["Status"].asString();
