...
ListZonesResult::ListZonesResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void ListZonesResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
...
std::vector<ListZonesResult::Zone> ListZonesResult::getZones()const
{
	return zones_;
}
