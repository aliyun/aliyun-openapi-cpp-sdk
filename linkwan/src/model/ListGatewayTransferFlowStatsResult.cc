...
ListGatewayTransferFlowStatsResult::ListGatewayTransferFlowStatsResult() :
	ServiceResult()
{}

ListGatewayTransferFlowStatsResult::ListGatewayTransferFlowStatsResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void ListGatewayTransferFlowStatsResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	for (auto valueDataFlowStat : allDataNode)
	{
		...
		if(!valueDataFlowStat["StatMillis"].isNull())
			dataObject.statMillis = valueDataFlowStat["StatMillis"].asString();
...
std::vector<ListGatewayTransferFlowStatsResult::FlowStat> ListGatewayTransferFlowStatsResult::getData()const
{
	return data_;
}

bool ListGatewayTransferFlowStatsResult::getSuccess()const
{
	return success_;
}
