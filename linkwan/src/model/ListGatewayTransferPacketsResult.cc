...
ListGatewayTransferPacketsResult::ListGatewayTransferPacketsResult() :
	ServiceResult()
{}

ListGatewayTransferPacketsResult::ListGatewayTransferPacketsResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void ListGatewayTransferPacketsResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	for (auto dataNodeListPacket : allListNode)
	{
		...
		if(!dataNodeListPacket["LogMillis"].isNull())
			packetObject.logMillis = dataNodeListPacket["LogMillis"].asString();
		...
		if(!dataNodeListPacket["Freq"].isNull())
			packetObject.freq = dataNodeListPacket["Freq"].asString();
		...
		if(!dataNodeListPacket["NodeOwnerAliyunId"].isNull())
			packetObject.nodeOwnerAliyunId = dataNodeListPacket["NodeOwnerAliyunId"].asString();
		auto allMacCommandCIDs = value["MacCommandCIDs"]["CID"];
		for (auto value : allMacCommandCIDs)
			packetObject.macCommandCIDs.push_back(value.asString());
...
ListGatewayTransferPacketsResult::Data ListGatewayTransferPacketsResult::getData()const
{
	return data_;
}

bool ListGatewayTransferPacketsResult::getSuccess()const
{
	return success_;
}
