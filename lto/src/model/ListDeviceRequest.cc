...
using AlibabaCloud::Lto::Model::ListDeviceRequest;

ListDeviceRequest::ListDeviceRequest()
    : RpcServiceRequest("lto", "2021-07-07", "ListDevice") {
...
std::string ListDeviceRequest::getRegionId() const {
  return regionId_;
}

void ListDeviceRequest::setRegionId(const std::string &regionId) {
  regionId_ = regionId;
  setParameter(std::string("RegionId"), regionId);
}

std::string ListDeviceRequest::getName() const {
  return name_;
}

void ListDeviceRequest::setName(const std::string &name) {
  name_ = name;
