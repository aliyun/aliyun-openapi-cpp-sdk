...
FuzzyQueryResult::FuzzyQueryResult() :
	ServiceResult()
{}

FuzzyQueryResult::FuzzyQueryResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void FuzzyQueryResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
...
std::string FuzzyQueryResult::getNextToken()const
{
	return nextToken_;
}

std::vector<FuzzyQueryResult::FilesItem> FuzzyQueryResult::getFiles()const
{
	return files_;
}
