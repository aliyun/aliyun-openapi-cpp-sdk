...
SimpleQueryResult::SimpleQueryResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void SimpleQueryResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	for (auto valueAggregationsAggregationsItem : allAggregationsNode)
	{
		...
		if(!valueAggregationsAggregationsItem["Value"].isNull())
			aggregationsObject.value = valueAggregationsAggregationsItem["Value"].asString();
...
std::string SimpleQueryResult::getNextToken()const
{
	return nextToken_;
}

std::vector<SimpleQueryResult::AggregationsItem> SimpleQueryResult::getAggregations()const
{
	return aggregations_;
}

std::vector<SimpleQueryResult::FilesItem> SimpleQueryResult::getFiles()const
{
	return files_;
}
