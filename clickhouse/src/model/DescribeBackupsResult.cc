...
void DescribeBackupsResult::parse(const std::string &payload)
{
	...
	for (auto valueItemsBackup : allItemsNode)
	{
		...
		if(!valueItemsBackup["BackupSetInfo"].isNull())
			itemsObject.backupSetInfo = valueItemsBackup["BackupSetInfo"].asString();
		...
		if(!valueItemsBackup["ExpireDate"].isNull())
			itemsObject.expireDate = valueItemsBackup["ExpireDate"].asString();
