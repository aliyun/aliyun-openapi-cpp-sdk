...
void DescribeUserEncryptionKeyListResult::parse(const std::string &payload)
{
	...
	auto dataNode = value["Data"];
		auto allKeyIds = dataNode["KeyIds"]["KeyId"];
		for (auto value : allKeyIds)
			data_.keyIds.push_back(value.asString());

}

DescribeUserEncryptionKeyListResult::Data DescribeUserEncryptionKeyListResult::getData()const
{
	return data_;
}
