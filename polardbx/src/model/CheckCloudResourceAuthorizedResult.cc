...
void CheckCloudResourceAuthorizedResult::parse(const std::string &payload)
{
	...
	if(!dataNode["AuthorizationState"].isNull())
		data_.authorizationState = dataNode["AuthorizationState"].asString();
	if(!dataNode["RoleArn"].isNull())
		data_.roleArn = dataNode["RoleArn"].asString();

}

CheckCloudResourceAuthorizedResult::Data CheckCloudResourceAuthorizedResult::getData()const
{
	return data_;
}
