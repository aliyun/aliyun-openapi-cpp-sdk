...
void DescribeDBInstanceSSLResult::parse(const std::string &payload)
{
	...
	if(!dataNode["SSLEnabled"].isNull())
		data_.sSLEnabled = dataNode["SSLEnabled"].asString() == "true";
	if(!dataNode["SSLExpiredTime"].isNull())
		data_.sSLExpiredTime = dataNode["SSLExpiredTime"].asString();
	if(!dataNode["CertCommonName"].isNull())
		data_.certCommonName = dataNode["CertCommonName"].asString();

}

DescribeDBInstanceSSLResult::Data DescribeDBInstanceSSLResult::getData()const
{
	return data_;
}
