...
GetOcTaxPunishmentResult::GetOcTaxPunishmentResult() :
	ServiceResult()
{}

GetOcTaxPunishmentResult::GetOcTaxPunishmentResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void GetOcTaxPunishmentResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
	...
	{
		DataItem dataObject;
		if(!valueDataDataItem["EventType"].isNull())
			dataObject.eventType = valueDataDataItem["EventType"].asString();
		...
		if(!valueDataDataItem["EventName"].isNull())
			dataObject.eventName = valueDataDataItem["EventName"].asString();
...
int GetOcTaxPunishmentResult::getTotalNum()const
{
	return totalNum_;
}

std::string GetOcTaxPunishmentResult::getMessage()const
{
	return message_;
}

int GetOcTaxPunishmentResult::getPageNum()const
{
	return pageNum_;
}

std::vector<GetOcTaxPunishmentResult::DataItem> GetOcTaxPunishmentResult::getData()const
{
	return data_;
}

int GetOcTaxPunishmentResult::getPageIndex()const
{
	return pageIndex_;
}

std::string GetOcTaxPunishmentResult::getCode()const
{
	return code_;
}

bool GetOcTaxPunishmentResult::getSuccess()const
{
	return success_;
}
