...
void UpdateTableAddColumnRequest::setColumn(const std::vector<Column>& column)
{
	column_ = column;
	for(int dep1 = 0; dep1!= column.size(); dep1++) {
		auto columnObj = column.at(dep1);
		std::string columnObjStr = "Column." + std::to_string(dep1 + 1);
		setParameter(columnObjStr + ".ColumnNameCn", columnObj.columnNameCn);
		setParameter(columnObjStr + ".Comment", columnObj.comment);
		setParameter(columnObjStr + ".ColumnName", columnObj.columnName);
		setParameter(columnObjStr + ".ColumnType", columnObj.columnType);
	}
}
