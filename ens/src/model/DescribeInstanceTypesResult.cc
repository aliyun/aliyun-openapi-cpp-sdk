...
void DescribeInstanceTypesResult::parse(const std::string &payload)
{
	...
	for (auto valueInstanceTypesInstanceType : allInstanceTypesNode)
	{
		...
		if(!valueInstanceTypesInstanceType["InstanceTypeName"].isNull())
			instanceTypesObject.instanceTypeName = valueInstanceTypesInstanceType["InstanceTypeName"].asString();
		if(!valueInstanceTypesInstanceType["MemorySize"].isNull())
			instanceTypesObject.memorySize = std::stoi(valueInstanceTypesInstanceType["MemorySize"].asString());
...
int DescribeInstanceTypesResult::getCode()const
{
	return code_;
}
