...
void DescribeInstanceMonitorDataResult::parse(const std::string &payload)
{
	...
	for (auto valueMonitorDataInstanceMonitorData : allMonitorDataNode)
	{
		...
		if(!valueMonitorDataInstanceMonitorData["CPU"].isNull())
			monitorDataObject.cPU = valueMonitorDataInstanceMonitorData["CPU"].asString();
		...
		if(!valueMonitorDataInstanceMonitorData["Memory"].isNull())
			monitorDataObject.memory = valueMonitorDataInstanceMonitorData["Memory"].asString();
...
int DescribeInstanceMonitorDataResult::getCode()const
{
	return code_;
}
