...
void ListResourcesResult::parse(const std::string &payload)
{
	...
	for (auto valueResourcesResource : allResourcesNode)
	{
		...
		if(!valueResourcesResource["Service"].isNull())
			resourcesObject.service = valueResourcesResource["Service"].asString();
		...
		if(!valueResourcesResource["CreateDate"].isNull())
			resourcesObject.createDate = valueResourcesResource["CreateDate"].asString();
...
std::vector<ListResourcesResult::Resource> ListResourcesResult::getResources()const
{
	return resources_;
}
