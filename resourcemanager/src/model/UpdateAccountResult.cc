...
UpdateAccountResult::UpdateAccountResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void UpdateAccountResult::parse(const std::string &payload)
{
	Json::Reader reader;
	Json::Value value;
...
UpdateAccountResult::Account UpdateAccountResult::getAccount()const
{
	return account_;
}
